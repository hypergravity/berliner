# %%
import numpy as np
from berliner.deprecated.machine import general_search_v2 as general_search
...
class Pepsi:
    ...
    def set_lnprior(self, lnprior=None):
        ...
        prior, e.g., MassFunction, galaxy / star counts model
    ...
    def preproc(self, sed_obs):
        ...
        if sed_obs.ndim == 1:
            ...
            for i in range(self.nband):
                ...
                if this_maglim is not None:
                    sed_obs[:, i] = np.where(
                        (sed_obs[:, i] >= this_maglim[0])
                        & (sed_obs[:, i] <= this_maglim[1]),
                        sed_obs[:, i],
                        np.nan,
                    )
    ...
    def evaluate(
        ...
        if sed_obs.ndim == 1:
            r = general_search(
                ...
                self.Alambda,
                sed_obs,
                sed_obs_err=sed_obs_err,
                vpi_obs=vpi_obs,
                vpi_obs_err=vpi_obs_err,
                Lvpi=Lvpi,
                Lprior=Lprior,
            ...
            for i in range(sed_obs.shape[0]):
                ...
                r = general_search(
                    ...
                    self.Alambda,
                    sed_obs[i],
                    sed_obs_err=sed_obs_err[i],
                    vpi_obs=vpi_obs[i],
                    vpi_obs_err=vpi_obs_err[i],
                    Lvpi=Lvpi,
                    Lprior=Lprior,
                    cost_order=cost_order,
                    av_llim=av_llim,
