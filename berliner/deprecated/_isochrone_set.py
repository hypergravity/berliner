...
from berliner.interpolate.isochrone_interp import isoc_linterp
...
class IsochroneGrid:
    ...
    def make_prior_sb(self, func_prior=None, qs=["logTe", "logG"]):
        if not self.delta_ready:
            ...
            w = func_prior(
                isoc_stacked["_lgage"], isoc_stacked["_mhini"], isoc_stacked["_lgmini"]
            )
            # d_volume
            dv = (
                isoc_stacked["_d_age"]
                * isoc_stacked["_d_mhini"]
...
def make_delta(x, scale="linear", loose=True):
    if scale == "linear":
        if loose:
            ...
            xedges = np.concatenate(([x[0]], x[:-1] + np.diff(x) / 2.0, [x[-1]]))
